...
namespace Client.Controllers
{
    ...
    public class CarController : Controller
    {
        ...
        public async Task<IActionResult> Index(Guid? id)
        {
            ...
            await _clientMessageHub.SendToServerMessage(new GetCompaniesRequest(), correlationId);
            var getCompaniesResponse = await _clientMessageHub.ReceiveFromServerMessage<GetCompaniesResponse>(correlationId);
        ...
        {
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new GetCarRequest(id), correlationId);
            var getCarResponse = await _clientMessageHub.ReceiveFromServerMessage<GetCarResponse>(correlationId);

            correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new GetCompanyRequest(getCarResponse.Car.CompanyId), correlationId);
            var getCompanyResponse = await _clientMessageHub.ReceiveFromServerMessage<GetCompanyResponse>(correlationId);
        ...
        public async Task<IActionResult> Create(string id)
        {
            ...
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new GetCompanyRequest(companyId), correlationId);
            var getCompanyResponse = await _clientMessageHub.ReceiveFromServerMessage<GetCompanyResponse>(correlationId);
                ...
                [Bind("CompanyId,VIN,RegNr,Online")] Car car)
        {
            ...
            car.Id = Guid.NewGuid();
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new CreateCarRequest(car), correlationId);
            var createCarResponse = await _clientMessageHub.ReceiveFromServerMessage<CreateCarResponse>(correlationId);
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new UpdateCarRequest(getCarResponse.Car), correlationId);
            var updateCarResponse = await _clientMessageHub.ReceiveFromServerMessage<UpdateCarResponse>(correlationId);
        ...
        public async Task<IActionResult> Edit(Guid id, [Bind("Id, Online")] Car car)
        {
            ...
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new GetCarRequest(id), correlationId);
            var oldCarResponse = await _clientMessageHub.ReceiveFromServerMessage<GetCarResponse>(correlationId);
        ...
        public async Task<IActionResult> DeleteConfirmed(Guid id)
        {
            ...
            correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new DeleteCarRequest(id), correlationId);
            var deleteCarResponse = await _clientMessageHub.ReceiveFromServerMessage<DeleteCarResponse>(correlationId);
