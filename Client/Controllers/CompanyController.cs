...
namespace CarClient.Controllers
{
    ...
    public class CompanyController : Controller
    {
        ...
        {
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new GetCompanyRequest(id), correlationId);
        ...
        public async Task<IActionResult> Create([Bind("Name,Address,CreationTime")] Company company)
        {
            ...
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new CreateCompanyRequest(company), correlationId);
            var creteCompanyResponse = await _clientMessageHub.ReceiveFromServerMessage<CreateCompanyResponse>(correlationId);
        ...
        public async Task<IActionResult> Edit(Guid id, [Bind("Id,CreationTime, Name, Address")] Company company)
        {
            ...
            correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new UpdateCompanyRequest(company), correlationId);
            var updateCompanyResponse = await _clientMessageHub.ReceiveFromServerMessage<UpdateCompanyResponse>(correlationId);
        ...
        {
            var correlationId = Guid.NewGuid();
            await _clientMessageHub.SendToServerMessage(new DeleteCompanyRequest(id), correlationId);
            var deleteCompanyResponse = await _clientMessageHub.ReceiveFromServerMessage<DeleteCompanyResponse>(correlationId);
